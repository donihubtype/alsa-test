...
const WEBCHAT_BUTTON_FILENAME = 'webchat-button.botonic.js'
const BOTONIC_TRIGGER_BUTTON_LIBRARY_NAME = 'BotonicTriggerButton'
...
function botonicTriggerButtonConfig(mode) {
  return {
    optimization: {
      minimizer: [terserPlugin],
    },
    ...
    target: 'web',
    entry: path.resolve(
      WEBPACK_ENTRIES_DIRNAME,
      WEBPACK_ENTRIES.WEBCHAT_BUTTON
    ),
    output: {
      path: OUTPUT_PATH,
      filename: WEBCHAT_BUTTON_FILENAME,
      library: BOTONIC_TRIGGER_BUTTON_LIBRARY_NAME,
      libraryTarget: UMD_LIBRARY_TARGET,
...
module.exports = function (env, argv) {
  if (env.target === BOTONIC_TARGET.ALL) {
    return [
      ...
      botonicTriggerButtonConfig(argv.mode),
    ]
